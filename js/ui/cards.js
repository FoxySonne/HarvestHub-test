...
function bindCard(card, index) {
  if (!card || card.dataset.collapsibleCardBound === "true" || shouldSkipCard(card)) return;
  ...
  const shouldCollapse = localStorage.getItem(stateKey) === "collapsed";
...
function bindCollapsibleCards() {
  ...
  Array.from(pageContent.querySelectorAll(".card")).forEach((card, index) => bindCard(card, index));
}
