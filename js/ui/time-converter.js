...
function readNumber(id) {
  const value = Number(String(getElement(id)?.value || "0").replace(",", "."));
...
function calculateTime() {
  const totalSeconds = readNumber(FIELD_IDS.days) * 86400 +
    readNumber(FIELD_IDS.hours) * 3600 +
    readNumber(FIELD_IDS.minutes) * 60 +
    readNumber(FIELD_IDS.seconds);
  const remainingSeconds = totalSeconds % 86400;
  return {
    totalMinutes: Math.floor(totalSeconds / 60),
    formatted: `${pad(Math.floor(totalSeconds / 86400))}д ${pad(Math.floor(remainingSeconds / 3600))}:${pad(Math.floor((remainingSeconds % 3600) / 60))}:${pad(remainingSeconds % 60)}`
  };
}
...
function bindTimeConverter() {
  const fields = Object.values(FIELD_IDS).map(getElement).filter(Boolean);
