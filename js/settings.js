...
function confirmLocalDataStorage() {
    ...
    return confirm(
        "Нажимая эту кнопку, ты соглашаешься, что HarvestHub будет сохранять профиль и введённые в калькуляторах значения в памяти браузера на этом устройстве.\n\n" +
        "Данные не отправляются на сервер и пока не синхронизируются между устройствами. Если открыть сайт на другом телефоне или компьютере, этот профиль там не появится автоматически.\n\n" +
        "Данные могут быть удалены, если нажать «Сбросить все данные» на сайте или очистить данные сайтов в настройках браузера."
    );
