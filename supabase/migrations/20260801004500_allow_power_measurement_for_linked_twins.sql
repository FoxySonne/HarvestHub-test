-- Разрешает владельцу основного участника открывать замеры его твинов.
-- Логика совпадает с get_alliance_squad_power() и save_alliance_squad_power().
...
begin
  ...
  select coalesce(p.linked_user_id, primary_participant.linked_user_id)
  into participant_owner
  from public.participants p
  left join public.participants primary_participant
    on primary_participant.id = p.primary_participant_id
   and primary_participant.alliance_id = p.alliance_id
   and primary_participant.member_status <> 'left'
   and primary_participant.purged_at is null
  ...
  if not (
    alliance_role in ('owner', 'r5', 'editor')
...
revoke execute on function public.get_alliance_squad_power_measurement(uuid, uuid, date) from public, anon;
