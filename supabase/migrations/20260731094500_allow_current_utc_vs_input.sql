...
begin
  ...
  if target_result_date is null
     or extract(isodow from target_result_date) not between 1 and 6 then
  ...
  if target_result_date > (now() at time zone 'UTC')::date then
    raise exception 'Нельзя сохранять результат VS за будущий UTC-день';
  end if;
...
revoke execute on function public.save_alliance_vs_result(uuid, uuid, date, numeric, boolean) from public, anon;
